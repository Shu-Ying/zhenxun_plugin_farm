...
class CSignInManager:
    @classmethod
    async def signInByUid(cls, uid: str) -> str:
        toDay = getToolManager().dateTime().date().today()
        message = ""
        ...
        if status == 1 or status == 2:
            ...
            message += config.g_sTranslation["signIn"]["success"].format(
                day=signDay, exp=exp, num=point
            )

            jSign = await getJsonManager().getSign()
            ...
            if reward:
                ...
                message += config.g_sTranslation["signIn"]["grandTotal"].format(
                ...
                if vipPoint > 0:
                    message += config.g_sTranslation["signIn"]["grandTotal1"].format(
                        num=vipPoint
                    )

                if plant:
                    for key, value in plant.items():
                        message += config.g_sTranslation["signIn"][
                            "grandTotal2"
                        ].format(name=key, num=value)
        else:
            message = config.g_sTranslation["signIn"]["error1"]
            pass
    ...
    async def createSignImage(
        ...
        """
        try:
            templatePath = config.g_sResourcePath / "html/sign_in.html"
            outputPath = config.g_sResourcePath / "temp_html/sign_in.html"
            savePath = config.DATA_PATH / "farm_res/html/sign_in.png"

            # 1. 生成渲染数据
            context = cls.generateSignData(
                todayRewards=todayRewards,
                cumulativeRewards=cumulativeRewards,
                currentProgress=currentProgress,
                totalMilestones=totalMilestones,
            ...
            # 3. 生成图片字节数据
            # imageBytes = await getToolManager().screenshotHtmlToBytes(outputPath)

            # 4. 保存图片文件
            await getToolManager().screenshotSave(
